...
unsigned char	I2C_TX_REC[21] = {"I2CTX CMD Recieved!"};
unsigned char	I2C_RX_REC[21] = {"I2CRX CMD Recieved!"};
...
RX_Loop:	
		...
		//Check UART Recieve String for "ACK"
		...
		//Check UART Recieve String for "INC"
		...
		//Check UART Recieve String for "OUC" - Output, PortC
		...
		//Check UART Recieve String for "OUB" - Output, PortC
		...
		//Check UART Recieve String for "IND"
		...
		//Check UART Recieve String for "OUD" - Output, PortD
				...
				if(RecWorld[2] == 0x43){
					if(RecWorld[3] == 0x54){
						if(RecWorld[4] == 0x58){
							...
							//Send the final UART messagee to PC saying that this transaction was successful.
							_flgUartFin = 0;
							uart_stop();
				...
				if(RecWorld[2] == 0x43){
					if(RecWorld[3] == 0x52){
						if(RecWorld[4] == 0x58){
							...
							//Now we can start the I2C Recieve based on the info from the PC App
							_flgI2CFin = 0;																	//reset I2C completed Flag
							i2c_stop();																		//Make sure I2C is not currently running
