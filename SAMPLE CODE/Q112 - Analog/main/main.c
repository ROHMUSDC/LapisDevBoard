//*****************************************************************************
// Program:	 LaPi Development Board Demo Code Q112-Analog
// Author:	 C. Schell & K. Bahar
//		 ROHM Semiconductor USA, LLC
//		 US Design Center
// Started:  Aug 2nd, 2013
...
//Carl's Notes...
//
//Usee the built-in sample/hold 10-bit successive approximation type A-D 
// converter on any of the 7 possible channels for the Q112 micro to measure 
// voltage on the input. Voltage source will be the center of a voltage divider 
// circuit with a 10k resistor on top to 5.0V, and a 10k-ohm NTC (Negative 
// Temperature Coefficient) Thermistor on bottom to Ground. At room temp, the
// voltage should be ~ 2.5V. With 10-bit A/D, over 5V range, our resolution 
// should be ~4.88mV per step.
//
//
//*****************************************************************************
...
//	PC7/AIN7	=> Q112 only
	...
	#define AnalogInputPin0 	PA0D	//Q112 (LQFP32 Package) Pin25 = PA0/AIN0 	
...
unsigned short saADC_Reading( char ADC_Channel ); //Return 10-bit A/D Value fron Channel x
//*****************************************************************************
...
unsigned short saADC_result;							//Used for ADC Conversion Results...
...
Loop:
	...
	//Function to init the built-in sample/hold 10-bit & then read and return ADVAL
	// uses onboard successive approximation type A-D converter module	
	Desired_saADC_chan = 0; 					// Channel X (0-7)
...
//START saADC_Reading...
//Started 8/8/2013
//Written for 10-bit Q100 series A/D - C. Schell 9 Aug 2013 => NOW WORKING!
unsigned short saADC_Reading(char ADC_Channel ) //Return 10-bit A/D Value fron Channel x
{
	unsigned char	reg;		//Register Check
	unsigned short	rl, rh, saADC_Adval;	//ADC Results (High & Low Bytes)
	int counterVar;
//Use the following procedure to operate the SA-ADC:
//	1. Before starting the SA-ADC, start oscillation of the high-speed clock 
//		(HSCLK) and wait until the oscillation stabilizes.
//	2. Set the SA-ADC mode register 0 (SADMOD0).
//	3. When bit 0 (SARUN) of SA-ADC control register 1 (SADCON1) is set to "1", 
//		the SA-ADC circuit becomes active and performs A/D conversion from 
//		the lower channel number that is selected in the SA-ADC mode register
//		(SADMOD0).
//	4. A/D conversion results are stored in the applicable SA-ADC result 
//		registers (SADRnL, SADRnH), and when A/D conversion of the largest 
//		channel number that is selected is terminated, an SA-ADC conversion 
//		termination interrupt (SADINT) is generated.
//	5. Finally, by using bit 0 (SALP) of the SADCON0 register, it is possible to 
//		specify whether to terminate A/D conversion (SARUN bit is "0") or restart
//		A/D conversion automatically at termination of A/D conversion of the 
//		last channel.


	//First set appropriate pin (based on Selected ADC_Channel) to Analog Input
	switch (ADC_Channel)
	{
	...
	//=== set value. === for 10-bit A/D - C. Schell 9 Aug 2013 => WORKING!
	saADC_Adval = (unsigned short)( (  rl >> 4 ) | ( rh << 2 ) );	//0 to 1023...
